...
def login(
    page: Page, base_url: str, username: str, password: str, timeout_ms: int
) -> tuple[bool, str]:
...
def run() -> int:
    ...
    parser.add_argument(
        "--timeout-ms", type=int, default=30000, help="Navigation timeout in milliseconds"
    )
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            ok, detail = login(
                page, base_url, args.seller_username.strip(), args.seller_password, args.timeout_ms
            )
            ...
            if ok:
                ...
                if "/accounts/login/" in page.url:
                    record(
                        "Seller access listing create", False, f"redirected to login: {page.url}"
                    )
                else:
                    ...
                    if not game_value:
                        ...
                        with page.expect_navigation(
                            wait_until="domcontentloaded", timeout=args.timeout_ms
                        ):
                            page.locator("form button[type='submit']").click()

                        url_ok = re.search(r"/listing/\d+/?$", page.url) is not None
                        title_ok = (
                            page.locator(".listing-detail-title", has_text=listing_title).count()
                            > 0
                        )
        ...
        try:
            if not listing_url:
                ...
                if ok:
                    ...
                    open_ok = (
                        page.locator("a.btn.btn-primary", has_text="Написать продавцу").count() > 0
                    )
                    ...
                    if open_ok:
                        ...
                        if not in_chat:
                            ...
                            message_ok = (
                                page.locator(".message-bubble-chat", has_text=message_text).count()
                                > 0
                            )
                            record(
        ...
        try:
            if not listing_url:
                record(
                    ...
                    False,
                    "listing URL is missing from seller scenario",
                )
            else:
                page.goto(listing_url, wait_until="domcontentloaded", timeout=args.timeout_ms)
                ...
                if buy_button.count() == 0:
                    record(
                        "Buyer create purchase request",
                        False,
                        "buy button not found on listing page",
                    )
                else:
                    with page.expect_navigation(
                        wait_until="domcontentloaded", timeout=args.timeout_ms
                    ...
                    on_form = (
                        re.search(r"/transactions/purchase-request/\d+/create/?$", page.url)
                        is not None
                    )
                    if not on_form:
                        record(
                            "Buyer create purchase request",
                            False,
                            f"unexpected form URL: {page.url}",
                        )
                    else:
                        ...
                        on_detail = (
                            re.search(r"/transactions/purchase-request/\d+/?$", page.url)
                            is not None
                        )
