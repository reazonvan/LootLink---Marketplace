...
class FAQ(models.Model):
    ...
    category = models.CharField(
        max_length=50, choices=CATEGORIES, default="general", verbose_name="Категория"
    )
...
class DescriptionTemplate(models.Model):
    ...
    game = models.ForeignKey(
        Game, on_delete=models.CASCADE, related_name="description_templates", verbose_name="Игра"
    )
    name = models.CharField(max_length=200, verbose_name="Название шаблона")
    template = models.TextField(
        verbose_name="Шаблон", help_text="Используйте {переменные} для подстановки"
    )
...
class AntiFraudRule(models.Model):
    ...
    rule_type = models.CharField(max_length=50, choices=RULE_TYPES, verbose_name="Тип правила")
    pattern = models.CharField(
        max_length=500, verbose_name="Паттерн", help_text="Email, телефон, IP и т.д."
    )
    risk_score = models.PositiveIntegerField(
        default=50, verbose_name="Оценка риска", help_text="0-100, где 100 = максимальный риск"
    )
