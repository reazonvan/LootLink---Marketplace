"""
Команда для создания типовых фильтров для категорий.
"""
...
class Command(BaseCommand):
    help = "Создает типовые фильтры для популярных категорий игр"
    ...
    def handle(self, *args, **options):  # noqa: C901
        ...
        # Фильтры по типам игр (типовые наборы)
        filters_data = {
            "CS2": {
                "Аккаунты": [
                    {
        ...
        for game_name, categories_data in filters_data.items():
            ...
            try:
                ...
                if not game:
                    self.stdout.write(
                        self.style.WARNING(f'  Игра "{game_name}" не найдена, пропускаем')
                    )
                    continue
                ...
                for category_name, filters in categories_data.items():
                    ...
                    category = Category.objects.filter(
                        game=game, name__icontains=category_name
                    ).first()
                    if not category:
