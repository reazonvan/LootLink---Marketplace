...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--to", type=str, help="Email получателя для теста")
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f'  Password: {"***" + settings.EMAIL_HOST_PASSWORD[-3:] if settings.EMAIL_HOST_PASSWORD else "(not set)"}\n'
        )
        ...
        if settings.EMAIL_BACKEND == "django.core.mail.backends.console.EmailBackend":
            ...
            if not options["check_only"]:
                self.stdout.write(
                    self.style.WARNING(
                        "Тестовое письмо будет выведено в консоль (не отправлено)...\n"
                    )
                )
            else:
                return
...
Команда LootLink
            ...
            self.stdout.write(self.style.ERROR(f"\nОШИБКА: {str(e)}\n"))
            self.stdout.write(
                self.style.WARNING("Проверьте логи: sudo journalctl -u lootlink -n 50\n")
            )
