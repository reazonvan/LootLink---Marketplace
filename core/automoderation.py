...
class AutoModerator:
    ...
    def check_text(self, text):
        ...
        if self.profanity_regex.search(text):
            violations.append({"type": "profanity", "message": "Обнаружена нецензурная лексика"})
            severity = "medium"
        ...
        if scam_matches:
            violations.append({"type": "scam", "message": "Обнаружены признаки мошенничества"})
            severity = "high"

        return {"is_clean": len(violations) == 0, "violations": violations, "severity": severity}
    ...
    def _log_violation(self, content_type, content_id, user, violations, severity):
        ...
        action = {"low": "flag", "medium": "warn", "high": "block"}.get(severity, "flag")
