...
class SecurityAuditLog(models.Model):
    ...
    action_type = models.CharField(
        max_length=50, choices=ACTION_TYPES, db_index=True, verbose_name="Тип действия"
    )
    ...
    # Детали действия
    description = models.TextField(verbose_name="Описание", help_text="Детальное описание действия")
    ...
    def get_suspicious_activity(cls, user, hours=24):
        ...
        return cls.objects.filter(
            user=user, created_at__gte=threshold, risk_level__in=["high", "critical"]
        )
    ...
    def get_failed_login_attempts(cls, ip_address, minutes=15):
        ...
        return cls.objects.filter(
            ip_address=ip_address, action_type="login_failed", created_at__gte=threshold
        ).count()
...
class DataChangeLog(models.Model):
    ...
    # Информация о записи
    model_name = models.CharField(max_length=100, db_index=True, verbose_name="Модель")
