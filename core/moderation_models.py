...
class UserBan(models.Model):
    ...
    user = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="bans", verbose_name="Пользователь"
    )
    ban_type = models.CharField(max_length=20, choices=BAN_TYPES, verbose_name="Тип бана")
    ...
    lifted_at = models.DateTimeField(null=True, blank=True, verbose_name="Дата снятия")
...
class UserWarning(models.Model):
    ...
    user = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="warnings", verbose_name="Пользователь"
    )
...
class AutoModeration(models.Model):
    ...
    content_type = models.CharField(
        max_length=50, verbose_name="Тип контента", help_text="listing, message, review и т.д."
    )
    ...
    reason = models.TextField(verbose_name="Причина")
    matched_patterns = models.JSONField(default=list, verbose_name="Обнаруженные паттерны")
...
class ModerationQueue(models.Model):
    ...
    content_type = models.CharField(
        max_length=50, choices=CONTENT_TYPES, verbose_name="Тип контента"
    )
    content_id = models.PositiveIntegerField(verbose_name="ID контента")
    user = models.ForeignKey(
        CustomUser,
    ...
    priority = models.IntegerField(
        default=0, verbose_name="Приоритет", help_text="Чем выше число, тем выше приоритет"
    )
    moderator = models.ForeignKey(
        CustomUser,
        on_delete=models.SET_NULL,
