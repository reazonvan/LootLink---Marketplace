...
from .moderation_models import AutoModeration, ModerationQueue, UserBan, UserWarning
...
def moderation_dashboard(request):
    ...
    # Последние действия
    recent_reports = Report.objects.select_related("reporter", "listing", "reported_user").order_by(
        "-created_at"
    )[:10]
...
def process_report(request, report_id):
    ...
    if request.method == "POST":
        ...
        if action == "approve":
            ...
            report.save()
            logger.info(
                "moderation report approved: report=%s moderator=%s",
                report.pk,
                request.user.pk,
            )
            ...
            report.save()
            logger.info(
                "moderation report rejected: report=%s moderator=%s",
                report.pk,
                request.user.pk,
            )
            ...
            reported_user = (
                report.reported_user if report.report_type == "user" else report.listing.seller
            )
            ...
            ban = UserBan.objects.create(
                ...
                expires_at=(
                    timezone.now() + timezone.timedelta(days=ban_days) if ban_days > 0 else None
                ),
            )
            ...
            report.save()
            logger.warning(
                "moderation user banned: ban=%s user=%s moderator=%s " "ban_days=%s report=%s",
                ban.pk,
                reported_user.pk,
                request.user.pk,
                ban_days,
...
def user_bans_list(request):
    ...
    context = {"bans": bans, "active_only": active_only}
