...
class ProfileAdmin(admin.ModelAdmin):
    ...
    def role_badge(self, obj):
        ...
        if obj.user.is_superuser:
            return format_html(
                '<span style="background: #dc2626; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: bold;">ВЛАДЕЛЕЦ</span>'
            )
        elif obj.user.is_staff:
            return format_html(
                '<span style="background: #2563eb; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: bold;">АДМИН</span>'
            )
        elif obj.is_moderator:
            return format_html(
                '<span style="background: #059669; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: bold;">МОДЕРАТОР</span>'
            )
        else:
            return format_html(
                '<span style="background: #9ca3af; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">ПОЛЬЗОВАТЕЛЬ</span>'
            )
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("user", "avatar", "bio")}),
        ("Контакты", {"fields": ("phone", "telegram", "discord")}),
        (
            "Права доступа",
            {
        ...
        (
            "Статистика",
            {"fields": ("rating", "total_sales", "total_purchases"), "classes": ("collapse",)},
        ),
    )
...
class PhoneVerificationAdmin(admin.ModelAdmin):
    ...
    def status_badge(self, obj):
        ...
        if obj.is_verified:
            return format_html(
                '<span style="background: #10b981; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">✔ Верифицирован</span>'
            )
        elif timezone.now() > obj.expires_at:
            return format_html(
                '<span style="background: #ef4444; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">⏱ Истек</span>'
            )
        elif obj.attempts >= 5:
            return format_html(
                '<span style="background: #f59e0b; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">⚠ Превышены попытки</span>'
            )
        else:
            return format_html(
                '<span style="background: #3b82f6; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">⏳ Ожидает</span>'
            )
...
class DocumentVerificationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Проверка", {"fields": ("status", "admin_comment", "reviewed_by", "reviewed_at")}),
    )
    ...
    def status_badge(self, obj):
        ...
        if obj.status == "approved":
            return format_html(
                '<span style="background: #10b981; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">✔ Одобрено</span>'
            )
        elif obj.status == "rejected":
            return format_html(
                '<span style="background: #ef4444; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">✘ Отклонено</span>'
            )
        else:
            return format_html(
                '<span style="background: #f59e0b; color: white; padding: 3px 8px; border-radius: 4px; font-size: 11px;">⏳ На проверке</span>'
            )
