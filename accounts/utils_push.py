...
def send_push_async(self, subscription_id, payload_json):
    ...
    sub = PushSubscription.objects.filter(id=subscription_id, is_active=True).first()
    if not sub:
        return
    ...
    try:
        ...
        logger.info(f"Push sent to subscription #{sub.id} (user={sub.user_id})")
    except WebPushException as e:
        # 410 Gone => подписка протухла, деактивируем без retry.
        if e.response is not None and e.response.status_code == 410:
            sub.is_active = False
            sub.save(update_fields=["is_active"])
            logger.info(f"Deactivated stale subscription #{sub.id} (410 Gone)")
            return
        logger.warning(f"WebPushException for subscription #{sub.id}: {e}")
        try:
            raise self.retry(exc=e)
        except self.MaxRetriesExceededError:
            logger.error(f"Max retries exceeded for push subscription #{sub.id}")
    except Exception as e:
        logger.exception(f"Unexpected error sending push to subscription #{sub.id}: {e}")
        try:
            raise self.retry(exc=e)
        except self.MaxRetriesExceededError:
            logger.error(f"Max retries exceeded (unexpected) for push subscription " f"#{sub.id}")
...
def send_push_notification(user, title, body, url=None, icon=None):
    ...
    for sub_id in subscription_ids:
        ...
        transaction.on_commit(lambda sid=sub_id: send_push_async.delay(sid, payload_json))
        queued += 1
