...
urlpatterns = [
    ...
    path("login/2fa/", views.login_2fa_verify, name="login_2fa"),
    path("logout/", views.user_logout, name="logout"),
    path("profile/<str:username>/", views.profile, name="profile"),
    path("profile-edit/", views.profile_edit, name="profile_edit"),
    path("settings/", views.account_settings, name="account_settings"),
    path("settings/terminate-session/", views.terminate_session, name="terminate_session"),
    path(
        "settings/terminate-all-sessions/",
    ...
    path(
        "verification/status/", verification_views.verification_status, name="verification_status"
    ),
    path(
        "verification/phone/request/",
        verification_views.phone_verification_request,
        name="phone_verification_request",
    ),
    path(
        "verification/phone/confirm/",
        verification_views.phone_verification_confirm,
        name="phone_verification_confirm",
    ),
    path(
        "verification/document/request/",
        verification_views.request_document_verification,
        name="request_document_verification",
    ),
    path(
        "verification/document/upload/",
        verification_views.document_verification_upload,
        name="document_verification_upload",
    ),
    path(
        "verification/document/status/",
        verification_views.document_verification_status,
        name="document_verification_status",
    ),
    # P0-17: защищённая раздача KYC-документов
    path(
        "verification/document/<int:document_id>/file/",
        verification_views.serve_kyc_document,
    ...
    path(
        "data-export/download/<int:export_id>/",
        views_export.download_data_export,
