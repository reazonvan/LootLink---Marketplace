...
from accounts.models import EmailVerification, PasswordResetCode, Profile
...
class TestCustomUserModel:
    ...
    def test_user_delete_is_soft(self):
        """delete() — soft-delete (анонимизация), не жёсткое удаление.

        P0-22: 152-ФЗ/GDPR требует возможности удалять данные, но
        финансовая история должна сохраняться. delete() анонимизирует
        PII и помечает is_deleted=True, но запись в БД остаётся.
        """
        user = CustomUser.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123"
        )
        ...
        assert user.email == f"deleted_{user_id}@deleted.lootlink.local"
        assert user.username == f"deleted_{user_id}"
        assert CustomUser.objects.filter(pk=user_id).exists()
...
class TestProfileModel:
    ...
    def test_profile_cannot_be_deleted_directly(self, verified_user):
        """Профиль нельзя удалить напрямую — только через soft-delete пользователя.

        P0-22: для не-удалённого пользователя delete() профиля бросает
        PermissionDenied. Удаление возможно только если user.is_deleted=True.
        """
    ...
    def test_profile_update_rating_with_reviews(
        self,
        verified_user,
        buyer,
        active_listing,
        ...
        P1-17: после фикса update_rating вызывается через transaction.on_commit,
        что в тестах внутри transaction-блока не срабатывает. Поэтому явно
        вызываем update_rating() после создания отзывов.
        """
