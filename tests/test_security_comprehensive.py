...
def moderator(db):
    user = User.objects.create_user(
        username="moderator", email="mod@test.com", password="modpass123", is_staff=True
    )
...
class TestSecurityAudit:
    ...
    def test_audit_log_creation(self, user):
        ...
        log = SecurityAuditLog.log(
            action_type="login_success", user=user, description="Test login", risk_level="low"
        )
...
class Test2FA:
    ...
    def test_totp_device_creation(self, user):
        ...
        device = TOTPDevice.objects.create(user=user, name="test-device", confirmed=False)
