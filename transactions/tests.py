...
class PurchaseRequestModelTest(TestCase):
    ...
    def test_unique_request_per_buyer_listing(self):
        ...
        PurchaseRequest.objects.create(listing=self.listing, buyer=self.buyer, seller=self.seller)
    ...
    def test_accept_purchase_request(self):
        """Тест принятия запроса (требует funded wallet после P0-10/P0-9)."""
    ...
    def test_complete_purchase_updates_statistics(self):
        ...
        request = PurchaseRequest.objects.create(
            listing=self.listing, buyer=self.buyer, seller=self.seller, status="accepted"
        )
...
class ReviewModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.game = Game.objects.create(name="Test Game", slug="test-game")
        self.listing = Listing.objects.create(
            seller=self.seller, game=self.game, title="Test Item", description="Test", price=100.00
        )
        self.purchase = PurchaseRequest.objects.create(
            listing=self.listing, buyer=self.buyer, seller=self.seller, status="completed"
        )
    ...
    def test_rating_updates_profile(self):
        ...
        P1-17: update_rating теперь через transaction.on_commit — в тестах
        внутри atomic-блока callback не срабатывает, вызываем явно.
        """
    ...
    def test_unique_review_per_user_purchase(self):
        ...
        Review.objects.create(
            purchase_request=self.purchase, reviewer=self.buyer, reviewed_user=self.seller, rating=5
        )
