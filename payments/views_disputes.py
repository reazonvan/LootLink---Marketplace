...
def create_dispute(request, escrow_id):
    ...
    if request.user not in [escrow.buyer, escrow.seller]:
        logger.warning(
            "payments IDOR attempt on create_dispute: user=%s escrow=%s buyer=%s seller=%s",
            request.user.pk,
            escrow.pk,
            escrow.buyer_id,
            escrow.seller_id,
    ...
    if escrow.status != "funded":
        logger.info(
            "create_dispute blocked (bad status): user=%s escrow=%s status=%s",
            request.user.pk,
            escrow.pk,
            escrow.status,
        )
    ...
    if hasattr(escrow, "dispute"):
        logger.info(
            "create_dispute blocked (already exists): user=%s escrow=%s dispute=%s",
            request.user.pk,
            escrow.pk,
            escrow.dispute.id,
        )
    ...
    if request.method == "POST":
        ...
        if not description or len(description) < 20:
            logger.info(
                "create_dispute rejected (short desc): user=%s escrow=%s len=%s",
                request.user.pk,
                escrow.pk,
                len(description),
            )
        ...
        with transaction.atomic():
            ...
            logger.warning(
                "dispute opened: id=%s escrow=%s opened_by=%s reason=%s buyer=%s seller=%s",
                dispute.id,
                escrow.pk,
                request.user.pk,
                reason,
...
def dispute_detail(request, dispute_id):
    ...
    if not (is_participant or is_moderator_user):
        logger.warning(
            "payments IDOR attempt on dispute_detail: user=%s dispute=%s",
            request.user.pk,
            dispute.pk,
        )
...
def add_dispute_message(request, dispute_id):
    ...
    if not (is_participant or is_moderator_user):
        logger.warning(
            "payments IDOR attempt on add_dispute_message: user=%s dispute=%s",
            request.user.pk,
            dispute.pk,
        )
    ...
    logger.info(
        "dispute message: id=%s dispute=%s sender=%s is_moderator=%s length=%s",
        dispute_message.id,
        dispute.pk,
        request.user.pk,
        is_moderator_user,
...
def moderate_dispute(request, dispute_id):
    ...
    if request.method == "POST":
        ...
        if not decision:
            logger.info(
                "moderate_dispute rejected (empty decision): moderator=%s dispute=%s action=%s",
                request.user.pk,
                dispute.pk,
                action,
            )
        ...
        try:
            if action == "resolve_buyer":
                ...
                logger.warning(
                    "moderate_dispute resolved_buyer: moderator=%s dispute=%s escrow=%s",
                    request.user.pk,
                    dispute.pk,
                    dispute.escrow_id,
                )
                ...
                logger.warning(
                    "moderate_dispute resolved_seller: moderator=%s dispute=%s escrow=%s",
                    request.user.pk,
                    dispute.pk,
                    dispute.escrow_id,
                )
                ...
                if refund_amount:
                    ...
                    dispute.save()
                    logger.warning(
                        "moderate_dispute resolved_partial: moderator=%s dispute=%s "
                        "escrow=%s refund=%s",
                        request.user.pk,
                        dispute.pk,
                        dispute.escrow_id,
                    ...
                    logger.info(
                        "moderate_dispute partial rejected (no amount): moderator=%s dispute=%s",
                        request.user.pk,
                        dispute.pk,
                    )
                ...
                dispute.save()
                logger.warning(
                    "moderate_dispute closed: moderator=%s dispute=%s",
                    request.user.pk,
                    dispute.pk,
                )
            ...
            logger.exception(
                "moderate_dispute failed: moderator=%s dispute=%s action=%s",
                request.user.pk,
                dispute.pk,
                action,
            )
