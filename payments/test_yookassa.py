...
class TestYookassaHandleWebhook:
    ...
    def test_succeeded_double_send_idempotent(self, verified_user):
        ...
        tx = self._make_pending_tx(verified_user, amount=Decimal("300.00"), payment_id="pid-2")
    ...
    def test_canceled_event(self, verified_user):
        ...
        payload = self._payload(
            tx, payment_id="pid-3", event="payment.canceled", status="canceled", paid=False
        )
        ...
        assert not Wallet.objects.filter(user=verified_user).exists() or Wallet.objects.get(
            user=verified_user
    ...
    def test_canceled_after_completed_skipped(self, verified_user):
        ...
        payload = self._payload(
            tx, payment_id="pid-4", event="payment.canceled", status="canceled", paid=False
        )
        result = service.handle_webhook(payload)
        assert result is True
        tx.refresh_from_db()
