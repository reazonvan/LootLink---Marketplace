...
def _set_wallet(user, balance, frozen=Decimal('0')):
    wallet, _ = Wallet.objects.get_or_create(
        user=user, defaults={'balance': balance, 'frozen_balance': frozen},
    )
...
def _create_escrow(buyer, seller, amount=Decimal('500.00'), funded=True,
                   deadline_offset_days=-1):
    """
    Создать Escrow для тестов.
    deadline_offset_days < 0 → дедлайн в прошлом (готов к auto-release).
    """
    game = Game.objects.create(name=f'G_{buyer.pk}_{seller.pk}',
                               slug=f'g-{buyer.pk}-{seller.pk}')
    listing = Listing.objects.create(
        seller=seller, game=game, title='Task Test Listing',
        description='for tasks', price=amount, status='active',
    )
    pr = PurchaseRequest.objects.create(
        listing=listing, buyer=buyer, seller=seller, status='accepted',
    )
    _set_wallet(buyer, balance=amount * 2)
    _set_wallet(seller, balance=Decimal('0'))
    escrow = Escrow.objects.create(
        purchase_request=pr, buyer=buyer, seller=seller, amount=amount,
    )
    if funded:
        escrow.fund()
...
class TestCheckPendingWithdrawals:
    ...
    def test_finds_old_pending(self, verified_user):
        ...
        w = Withdrawal.objects.create(
            user=verified_user, amount=Decimal('500.00'),
            payment_method='card', payment_details='1111 2222 3333 4444',
            status='pending',
        )
    ...
    def test_ignores_recent_pending(self, verified_user):
        ...
        Withdrawal.objects.create(
            user=verified_user, amount=Decimal('300'),
            payment_method='card', payment_details='1111',
            status='pending',
        )
        result = check_pending_withdrawals()
        assert result['count'] == 0
...
class TestCleanupOldTransactions:
    ...
    def test_finds_old_completed(self, verified_user):
        tx = Transaction.objects.create(
            user=verified_user, transaction_type='deposit',
