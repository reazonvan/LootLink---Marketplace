...
def auto_release_escrow(self):
    ...
    for escrow_id in expired_ids:
        try:
            ...
            skipped_count += 1
            logger.warning(f"Escrow #{escrow_id} исчез между select и select_for_update")
            continue

        except Exception as e:
            errors_count += 1
            last_exception = e
            logger.exception(f"Error auto-releasing escrow #{escrow_id}: {str(e)}")
            ...
            try:
                with transaction.atomic():
                    SecurityAuditLog.log(
                        ...
                        user=None,
                        description=(
                            f"Ошибка автоматического освобождения " f"escrow #{escrow_id}: {str(e)}"
                        ),
                        risk_level="high",
                        metadata={
                            "escrow_id": escrow_id,
    ...
    if errors_count and last_exception is not None:
        try:
            ...
            logger.warning(
                "auto_release_escrow: исчерпан лимит retry, " f"errors_count={errors_count}"
            )
...
def cleanup_old_transactions():
    ...
    old_transactions = Transaction.objects.filter(created_at__lte=threshold, status="completed")
