...
class TestConversationsList:
    ...
    def test_conversations_list_shows_conversations(
        self, authenticated_client, verified_user, seller, conversation_factory
    ):
...
class TestConversationDetail:
    ...
    def test_conversation_detail_requires_login(self, client, seller, buyer, conversation_factory):
        ...
        response = client.get(reverse("chat:conversation_detail", kwargs={"pk": conversation.pk}))
        assert response.status_code == 302

    def test_conversation_detail_loads(
        self, authenticated_client, verified_user, seller, conversation_factory
    ):
    ...
    def test_conversation_only_for_participants(
        self, authenticated_client, verified_user, seller, buyer, conversation_factory
    ):
    ...
    def test_send_message_success(
        ...
        response = authenticated_client.post(
            reverse("chat:conversation_detail", kwargs={"pk": conversation.pk}), data
        )

        assert Message.objects.filter(
            conversation=conversation, sender=verified_user, content="Test message"
        ).exists()

    def test_messages_marked_read(
        self, authenticated_client, verified_user, seller, conversation_factory, message_factory
    ):
...
class TestConversationStart:
    ...
    def test_start_conversation_success(self, authenticated_client, verified_user, active_listing):
        ...
        assert response.status_code == 302
        assert Conversation.objects.filter(listing=active_listing).exists()

    def test_start_conversation_redirects_to_existing(
        self, authenticated_client, verified_user, active_listing, conversation_factory
    ):
...
class TestGetNewMessages:
    ...
    def test_get_new_messages_success(
        self, authenticated_client, verified_user, seller, conversation_factory, message_factory
    ):
