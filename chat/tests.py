...
class ConversationModelTest(TestCase):
    ...
    def test_conversation_creation(self):
        ...
        conversation = Conversation.objects.create(
            participant1=self.user1, participant2=self.user2, listing=self.listing
        )
    ...
    def test_get_unread_count(self):
        ...
        Message.objects.create(
            conversation=conversation, sender=self.user2, content="Hello!", is_read=False
        )
        Message.objects.create(
            conversation=conversation, sender=self.user2, content="How are you?", is_read=False
        )
...
class MessageModelTest(TestCase):
    ...
    def test_message_creation(self):
        ...
        message = Message.objects.create(
            conversation=self.conversation, sender=self.user1, content="Hello, world!"
        )
...
class ChatViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.game = Game.objects.create(name="Test Game", slug="test-game")
        self.listing = Listing.objects.create(
            seller=self.user2, game=self.game, title="Test Item", description="Test", price=100.00
        )
    ...
    def test_cannot_start_conversation_with_self(self):
        ...
        self.assertFalse(
            Conversation.objects.filter(participant1=self.user2, participant2=self.user2).exists()
        )
    ...
    def test_send_message(self):
        ...
        self.assertTrue(
            Message.objects.filter(
                conversation=conversation, sender=self.user1, content="Test message"
            ).exists()
        )
    ...
    def test_messages_marked_as_read(self):
        ...
        conversation = Conversation.objects.create(participant1=self.user1, participant2=self.user2)
        message = Message.objects.create(
            conversation=conversation, sender=self.user2, content="Unread message", is_read=False
        )
