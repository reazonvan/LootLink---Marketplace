...
urlpatterns = [
    ...
    path(
        "api/users/<int:user_id>/moderator/",
        api_views.toggle_moderator,
        name="api_toggle_moderator",
    ),
    # Объявления
    path(
        "api/listings/<int:listing_id>/approve/",
        api_views.approve_listing,
        name="api_approve_listing",
    ),
    path(
        "api/listings/<int:listing_id>/reject/", api_views.reject_listing, name="api_reject_listing"
    ),
    path(
        "api/listings/<int:listing_id>/delete/", api_views.delete_listing, name="api_delete_listing"
    ),
    # Сделки
    path(
        "api/transactions/<int:transaction_id>/cancel/",
        api_views.cancel_transaction,
        name="api_cancel_transaction",
    ),
    # Споры
    path(
        "api/disputes/<int:dispute_id>/resolve/",
        api_views.resolve_dispute,
        name="api_resolve_dispute",
    ),
    # Репорты
    path(
        "api/reports/<int:report_id>/process/", api_views.process_report, name="api_process_report"
    ),
    # Статистика
