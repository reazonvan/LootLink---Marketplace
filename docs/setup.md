...
Документ описывает запуск проекта на чистой машине под Windows, Linux или macOS.
Если у вас уже есть Python 3.13 и PostgreSQL — переходите сразу к шагу 4.
...
Доступны каталог, создание объявлений, запросы на покупку, чат и отзывы.
...
Если что-то не запускается, проверяйте по списку:

1. PostgreSQL запущен и доступен на `localhost:5432`.
2. Виртуальное окружение активировано (`(venv)` в начале строки).
3. Зависимости установлены без ошибок.
4. `.env` существует и содержит корректные `DB_*` и `SECRET_KEY`.
5. Миграции применены (`python manage.py showmigrations` показывает `[X]`).
