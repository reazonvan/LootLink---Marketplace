...
LootLink — web-приложение на Django с server-side рендерингом (HTML-страницы и
формы). Поверх него есть **REST API на DRF** (`/api/...`) для программного
доступа и мобильных клиентов — см. раздел [REST API (DRF)](#rest-api-drf).

Ниже описаны HTTP-эндпоинты страниц/форм, REST API и служебные endpoints
(health, метрики).
...
**URL:** `/transactions/purchase-request/<id>/confirm-received/`
**Метод:** `POST`
**Доступ:** Только покупатель

**Результат:**
- Статус меняется на `completed`, объявление помечается как проданное
- Эскроу освобождается продавцу за вычетом комиссии платформы
  (`PLATFORM_COMMISSION_PERCENT`)
- Обновляется статистика и рейтинги участников

> Завершение сделки идёт через подтверждение покупателя (или авто-release
> эскроу по дедлайну через Celery beat). Продавец **не** может в одностороннем
> порядке «завершить» сделку и забрать средства — это защита покупателя.
...
Базовый префикс `/api/`. Формат — JSON. Аутентификация: Session (для браузера)
или Token (`Authorization: Token <key>`). Пагинация — по 20
(`PageNumberPagination`, параметр `page`). Троттлинг: анонимные — 100/час,
авторизованные — 1000/час. Права по умолчанию — `IsAuthenticatedOrReadOnly`
(чтение публичное, запись требует входа).

| URL | Методы | Описание |
|-----|--------|----------|
| `/api/games/` | GET | Игры (read-only) |
| `/api/categories/` | GET | Категории (read-only) |
| `/api/listings/` | GET, POST, PUT, PATCH, DELETE | Объявления (полный CRUD); фильтры `game`, `search`, `ordering` |
| `/api/reviews/` | GET, POST, PUT, PATCH, DELETE | Отзывы (полный CRUD) |
| `/api/conversations/` | GET | Диалоги текущего пользователя (read-only) |
| `/api/auth/token/` | POST | Получить токен по `username` + `password` |
| `/api/push/vapid-public-key/` | GET | Публичный VAPID-ключ для Web Push |
...
| `/health/ready/` | Readiness — доступны БД и кэш |
| `/metrics/` | Prometheus-метрики (Bearer-токен или staff; снаружи Caddy отдаёт 403) |
| `/sitemap.xml` · `/robots.txt` · `/manifest.json` | SEO и PWA |
