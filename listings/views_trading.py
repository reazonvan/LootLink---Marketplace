...
from .models_trading import ListingReservation, PriceHistory, PriceOffer
...
def make_price_offer(request, listing_id):
    ...
    if listing.seller == request.user:
        logger.info(
            "price_offer blocked (self): user=%s listing=%s",
            request.user.pk,
            listing.pk,
        )
    ...
    try:
        ...
        if offered_price <= 0:
            logger.info(
                "price_offer rejected (non-positive): buyer=%s listing=%s offered=%s",
                request.user.pk,
                listing.pk,
                offered_price,
            )
        ...
        if offered_price >= listing.price:
            logger.info(
                "price_offer rejected (not lower): buyer=%s listing=%s offered=%s asked=%s",
                request.user.pk,
                listing.pk,
                offered_price,
                listing.price,
        ...
        existing_offer = PriceOffer.objects.filter(
            listing=listing, buyer=request.user, status="pending"
        ).first()

        if existing_offer:
            logger.info(
                "price_offer rejected (active exists): buyer=%s listing=%s existing=%s",
                request.user.pk,
                listing.pk,
                existing_offer.pk,
            )
        ...
        logger.info(
            "price_offer created: id=%s buyer=%s listing=%s offered=%s original=%s",
            offer.pk,
            request.user.pk,
            listing.pk,
            offered_price,
        ...
        return JsonResponse(
            {"success": True, "message": "Предложение отправлено продавцу", "offer_id": offer.id}
        )

    except (ValueError, TypeError):
        logger.info(
            "price_offer rejected (bad decimal): buyer=%s listing=%s raw=%r",
            request.user.pk,
            listing.pk,
            request.POST.get("offered_price"),
        )
...
def my_price_offers(request):
    ...
    offers_made = (
        PriceOffer.objects.filter(buyer=request.user)
    ...
    offers_received = (
        PriceOffer.objects.filter(seller=request.user)
...
def respond_to_offer(request, offer_id):
    ...
    if offer.seller != request.user:
        logger.warning(
            "listings IDOR attempt on respond_to_offer: user=%s offer=%s seller=%s",
            request.user.pk,
            offer.pk,
            offer.seller_id,
        )
    ...
    if action == "accept":
        offer.accept()
        logger.info(
            "price_offer accepted: id=%s seller=%s offered=%s",
            offer.pk,
            request.user.pk,
            offer.offered_price,
        )
        ...
        offer.reject()
        logger.info(
            "price_offer rejected by seller: id=%s seller=%s",
            offer.pk,
            request.user.pk,
        )
        ...
        try:
            ...
            logger.info(
                "price_offer counter: id=%s seller=%s counter=%s",
                offer.pk,
                request.user.pk,
                counter_price,
            )
            ...
            logger.info(
                "price_offer counter rejected (bad decimal): id=%s seller=%s raw=%r",
                offer.pk,
                request.user.pk,
                request.POST.get("counter_price"),
            )
            return JsonResponse({"success": False, "error": "Неверная цена"})

    logger.info(
        "respond_to_offer unknown action: seller=%s offer=%s action=%r",
        request.user.pk,
        offer.pk,
        action,
    )
...
def reserve_listing(request, listing_id):
    ...
    if listing.seller == request.user:
        logger.info(
            "reserve_listing blocked (self): user=%s listing=%s",
            request.user.pk,
            listing.pk,
        )
    ...
    existing = ListingReservation.objects.filter(
        listing=listing, buyer=request.user, status="active"
    ).first()

    if existing and existing.is_active():
        logger.info(
            "reserve_listing blocked (duplicate): user=%s listing=%s existing=%s",
            request.user.pk,
            listing.pk,
            existing.pk,
        )
    ...
    reservation = ListingReservation.objects.create(
        listing=listing, buyer=request.user, duration_hours=duration_hours
    )
    ...
    logger.info(
        "reservation created: id=%s buyer=%s listing=%s duration_hours=%s expires_at=%s",
        reservation.pk,
        request.user.pk,
        listing.pk,
        duration_hours,
...
def my_reservations(request):
    """Мои резервирования"""
    reservations = (
        ListingReservation.objects.filter(buyer=request.user)
...
def cancel_reservation(request, reservation_id):
    ...
    if reservation.buyer != request.user:
        logger.warning(
            "listings IDOR attempt on cancel_reservation: user=%s reservation=%s buyer=%s",
            request.user.pk,
            reservation.pk,
    ...
    reservation.cancel()
    logger.info(
        "reservation cancelled by buyer: id=%s buyer=%s listing=%s",
        reservation.pk,
        request.user.pk,
...
def listing_price_history(request, listing_id):
    ...
    price_history = PriceHistory.objects.filter(listing=listing).order_by("-changed_at")

    context = {"listing": listing, "price_history": price_history}
