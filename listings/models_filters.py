...
class CategoryFilter(models.Model):
    ...
    filter_type = models.CharField(
        max_length=20, choices=FILTER_TYPES, default="select", verbose_name="Тип фильтра"
    )
...
class FilterOption(models.Model):
    ...
    filter = models.ForeignKey(
        CategoryFilter, on_delete=models.CASCADE, related_name="options", verbose_name="Фильтр"
    )
    value = models.CharField(
        max_length=100, verbose_name="Значение", help_text="Например: Global Elite, Arcana, 30000+"
    )
...
class ListingFilterValue(models.Model):
    ...
    listing = models.ForeignKey(
        "Listing", on_delete=models.CASCADE, related_name="filter_values", verbose_name="Объявление"
    )
    ...
    value_text = models.CharField(max_length=200, blank=True, verbose_name="Текстовое значение")
    value_int = models.IntegerField(null=True, blank=True, verbose_name="Числовое значение")
    value_bool = models.BooleanField(null=True, blank=True, verbose_name="Булевое значение")
