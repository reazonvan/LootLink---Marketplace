...
from listings.models import Favorite, Game, Listing, Report
...
class TestCatalog:
    ...
    def test_catalog_filters_by_price(self, client, seller, listing_factory):
        ...
        response = client.get(reverse("listings:catalog"), {"min_price": "100", "max_price": "600"})
    ...
    def test_catalog_pagination(self, client, seller, listing_factory):
        """Каталог — алфавитный список игр, пагинации нет."""
        for i in range(15):
...
class TestListingDetail:
    ...
    def test_listing_detail_loads(self, client, active_listing):
        ...
        response = client.get(reverse("listings:listing_detail", kwargs={"pk": active_listing.pk}))
        assert response.status_code == 200
    ...
    def test_listing_detail_not_found(self, client):
        ...
        response = client.get(reverse("listings:listing_detail", kwargs={"pk": 99999}))
        assert response.status_code == 404

    def test_user_has_request_shown(
...
class TestListingCreate:
    ...
    def test_create_listing_max_limit(
        self, authenticated_client, verified_user, game, listing_factory
    ):
...
class TestListingEdit:
    ...
    def test_edit_requires_login(self, client, active_listing):
        ...
        response = client.get(reverse("listings:listing_edit", kwargs={"pk": active_listing.pk}))
        assert response.status_code == 302
    ...
    def test_edit_listing_success(self, authenticated_client, verified_user, game, listing_factory):
        ...
        response = authenticated_client.post(
            reverse("listings:listing_edit", kwargs={"pk": listing.pk}), data
        )
...
class TestListingDelete:
    ...
    def test_delete_requires_login(self, client, active_listing):
        ...
        response = client.get(reverse("listings:listing_delete", kwargs={"pk": active_listing.pk}))
        assert response.status_code == 302
    ...
    def test_cannot_delete_with_active_requests(
...
class TestFavorites:
    ...
    def test_toggle_favorite_add(self, authenticated_client, verified_user, active_listing):
        ...
        assert Favorite.objects.filter(user=verified_user, listing=active_listing).exists()
    ...
    def test_toggle_favorite_remove(self, authenticated_client, verified_user, active_listing):
        ...
        assert not Favorite.objects.filter(user=verified_user, listing=active_listing).exists()
...
class TestReports:
    ...
    def test_report_listing_requires_login(self, client, active_listing):
        ...
        response = client.get(reverse("listings:report_listing", kwargs={"pk": active_listing.pk}))
        assert response.status_code == 302
    ...
    def test_report_listing_success(self, authenticated_client, verified_user, active_listing):
        ...
        response = authenticated_client.post(
            reverse("listings:report_listing", kwargs={"pk": active_listing.pk}), data
        )

        assert response.status_code == 302
        assert Report.objects.filter(reporter=verified_user, listing=active_listing).exists()
    ...
    def test_report_user_success(self, authenticated_client, verified_user, seller):
        ...
        response = authenticated_client.post(
            reverse("listings:report_user", kwargs={"username": seller.username}), data
        )

        assert response.status_code == 302
        assert Report.objects.filter(reporter=verified_user, reported_user=seller).exists()
...
class TestGameListings:
    ...
    def test_game_listings_loads(self, client, game):
        ...
        response = client.get(reverse("listings:game_listings", kwargs={"game_slug": game.slug}))
        assert response.status_code == 200
        assert game.name in response.content.decode()

    def test_game_listings_shows_only_game_listings(
        self, client, game, game_factory, seller, listing_factory
    ):
