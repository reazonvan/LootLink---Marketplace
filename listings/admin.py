...
class GameAdmin(admin.ModelAdmin):
    ...
    def categories_count(self, obj):
        ...
        return format_html(
            '<span style="background: #eff6ff; color: #2563eb; padding: 2px 8px; border-radius: 4px; font-weight: bold;">{}</span>',
            count,
        )
    ...
    def listings_count(self, obj):
        ...
        if count > 0:
            return format_html(
                '<span style="background: #10b981; color: white; padding: 2px 8px; border-radius: 4px; font-weight: bold;">{}</span>',
                count,
            )
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("name", "slug", "description", "icon")}),
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("game", "name", "slug", "description")}),
        (
            "Оформление",
            {
                "fields": ("icon",),
...
class ReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Обработка", {"fields": ("status", "admin_comment", "resolved_at")}),
