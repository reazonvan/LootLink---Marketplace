...
class PriceHistory(models.Model):
    ...
    listing = models.ForeignKey(
        Listing, on_delete=models.CASCADE, related_name="price_history", verbose_name="Объявление"
    )
...
class PriceOffer(models.Model):
    ...
    listing = models.ForeignKey(
        Listing, on_delete=models.CASCADE, related_name="price_offers", verbose_name="Объявление"
    )
    ...
    message = models.TextField(
        blank=True, max_length=500, verbose_name="Сообщение", help_text="Почему такая цена?"
    )
    ...
    counter_price = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="Встречная цена"
    )
    ...
    def accept(self):
        ...
        PriceHistory.objects.create(
            listing=self.listing, old_price=self.original_price, new_price=self.offered_price
        )
...
class ListingReservation(models.Model):
    ...
    listing = models.ForeignKey(
        Listing, on_delete=models.CASCADE, related_name="reservations", verbose_name="Объявление"
    )
    buyer = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="reservations", verbose_name="Покупатель"
    )

    duration_hours = models.PositiveIntegerField(
        default=24, verbose_name="Длительность (часы)", help_text="Сколько часов зарезервировать"
    )
    status = models.CharField(
